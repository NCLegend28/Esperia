...
def extract_pdf(path: Path) -> str:
    """Extract up to 300 pages with page markers and an explicit size bound."""
    reader = PdfReader(path)
    if reader.is_encrypted:
        raise ExtractionLimit("Encrypted PDF is unsupported")
    if len(reader.pages) > 300:
        raise ExtractionLimit("PDF exceeds 300-page extraction limit")
    parts: list[str] = []
    total = 0
    for number, page in enumerate(reader.pages, 1):
        contents = page.get_contents()
        if contents is not None and len(contents.get_data()) > 20_000_000:
    ...
    if sum(len(p.split("]", 1)[-1].strip()) for p in parts) < 100:
...
if __name__ == "__main__":
    try:
        print(json.dumps({"text": extract_pdf(Path(sys.argv[1]))}))
