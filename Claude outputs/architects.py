...
PART_TYPES = ("plinth", "volume", "fins", "slabs", "pylons", "strip", "mast", "facade_lights", "cylinder")
...
def coherence(spec: Spec, warnings: list[str]) -> float:
    ...
    if spec.height() < 3:
        score -= 0.3
    if not any(p.type in ("volume", "fins", "slabs", "cylinder") for p in spec.parts):
...
def _template_part(rng: random.Random, top: float, palette: dict[str, str]) -> JSON:
    ...
    kind = rng.choice(["volume", "strip", "mast", "fins", "pylons", "facade_lights", "cylinder", "slabs"])
...
def mutate(data: JSON, rng: random.Random, donors: list[JSON], strength: int = 3) -> JSON:
    ...
    for _ in range(strength):
        ...
        if op == "num" and body:
            ...
            p = rng.choice(body)
            keys = [k for k in ("slot", "edges", "corner", "cap", "beacon", "alt", "foot") if k in p and p[k]]
            if keys:
                k = rng.choice(keys)
            ...
            if candidates and len(parts) < 14:
                ...
                top = max((float(p.get("y", 0)) + float(p.get("h", 0)) for p in parts), default=1.2)
                if part.get("type") in ("volume", "fins", "slabs", "cylinder", "mast", "pylons"):
            ...
            p = rng.choice(body)
            p["x"] = round(max(-4.0, min(4.0, float(p.get("x", 0)) + rng.uniform(-3.5, 3.5))), 2)
            p["z"] = round(max(-4.0, min(4.0, float(p.get("z", 0)) + rng.uniform(-3.5, 3.5))), 2)
    d["parts"] = parts
    return d
...
def crossover(a: JSON, b: JSON, rng: random.Random) -> JSON:
    ...
    for p in pb[len(pb) // 2 :]:
        q = copy.deepcopy(p)
        if q.get("type") in ("volume", "fins", "slabs", "cylinder", "mast", "pylons"):
...
def llm_proposer(url: str, model: str, timeout: float = 120.0) -> Proposer:
    ...
    def propose(architect: Architect, brief: JSON, kit: dict[str, JSON], rng: random.Random) -> list[JSON]:
        ...
        spec["statement"] = str(obj.get("statement", ""))[:200] if isinstance(obj, dict) else ""
